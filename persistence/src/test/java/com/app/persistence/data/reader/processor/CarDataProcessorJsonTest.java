...
class CarDataProcessorJsonTest {

    @Autowired
    CarDataProcessor carDataJsonProcessor;

    @Test
    @DisplayName("When data source is json")
    void test1() {
        ...
        var path = Paths
                .get("src", "test", "data", "test-cars.json")
                .toFile()
                .getAbsolutePath();

        assertThat(carDataJsonProcessor.process(path))
                .hasSize(2)
